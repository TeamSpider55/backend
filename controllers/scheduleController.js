...
let scheduleController1 = {
  ...
  retrieveSchedule: async (unixTime, concreteUser) => {
    let errMess = `No schedule ${new Date(
      Util.extractUnixOfYYYY_MM_DD(unixTime)
    )}!!!`;
    let schedule = null;
    try {
  ...
  removeAllScheduleBetween: async (concreteUser, start, end) => {
    let errMess =
      "Error: occur when delete schedule, of id:" +
      concreteUser +
      " between the indicate period!!!!";
    let report = null;
    try {
      report = await Schedule.deleteMany({
        $and: [
