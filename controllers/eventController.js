...
let eventController = {
    ...
    */
    retrieveSortedEventsInDay: async (unixTime, user, isSort) => {
        ...
        return (response.statusCode == 400)
            ...
            : await eventController.retrieveSortedEventsInSchedule(response.data, isSort);
    },
    ...
    */
    modifiedEventTime: async (unixStart, unixEnd, unixNewStart, unixNewEnd, user) => {
        let flag = 400;
        ...
        if(eventRes.statusCode == 200){
            ...
            if(checkDateVal(start, newStart) && checkDateVal(end, newEnd)){
                if((await eventController.removeEvent(eventRes.data, user)).statusCode){

                    eventRes.data.start = newStart;
                    eventRes.data.end = newEnd;
                    if((await eventController.AddEvent(eventRes.data, user)).statusCode){
                        flag = 200;
                    }else {
                        event.start = start;
                        event.end = start;
    ...
    */
    rescheduleEvent: async  (unixStart,unixEnd,unixNewStart,unixNewEnd, user) => {
        return(unixNewStart < Date.now() || unixNewEnd < Date.now())
            ? await eventController.modifiedEventTime(unixStart, unixEnd, unixNewStart, unixNewEnd, user)
