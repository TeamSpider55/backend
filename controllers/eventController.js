const ScheduleController = require("./scheduleController1");
...
let eventController = {
    ...
    ValidateEvent: (event, existEvents) => {
        ...
        if(existEvents.length > 0){
            for(var exist of existEvents){
                if((event.end > exist.start && event.start <= exist.end) ||
    ...
    retrieveSortedEventsInDay: async (unixTime, user, isSort) => {
        ...
        if(schedule != null)
            return await eventController.retrieveSortedEventsInSchedule(schedule, isSort);
        else
            return [];
    },
    ...
    rescheduleEvent: async (unixStart, unixEnd, unixNewStart, unixNewEnd, user) => {
        ...
        if(Util.extractUnixOfYYYY_MM_DD(start) == Util.extractUnixOfYYYY_MM_DD(newStart) &&
    ...
    modifyEventContent: async (newEvent, oldEvent, user) => {
        ...
        for(var i=0; i < schedule.events.length; i++){
            ...
            if(schedule.events[i].start == oldEvent.start && schedule.events[i].end == oldEvent.end){
                ...
                schedule.events[i].category = (newEvent.category != null) ? newEvent.category : schedule.events[i].category;
                await schedule.save();
                return true;
            }
        }return false;
